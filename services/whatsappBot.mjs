    ...
    for (const m of messages) {
      try {
        ...
        const timestamp = m.messageTimestamp
          ? (typeof m.messageTimestamp === 'number' ? m.messageTimestamp : m.messageTimestamp.low)
          : Math.floor(Date.now() / 1000);
        const fromMe = m.key.fromMe ? 1 : 0;
        const id = m.key.id;

        // Simpan pesan ke database lokal SQLite
        try {
          const db = require('../config/database.js');
          const name = m.pushName || '';
          let unread = 0;
          if (!fromMe) {
            const existing = db.prepare('SELECT unread_count FROM whatsapp_chats WHERE jid = ?').get(remote);
          ...
          `).run(remote, name, unread, text, timestamp);

          db.prepare(`
            INSERT OR IGNORE INTO whatsapp_messages (id, chat_jid, from_me, text, timestamp)
            VALUES (?, ?, ?, ?, ?)
          `).run(id, remote, fromMe, text, timestamp);
        } catch (dbErr) {
          logger.error('[WA DB Log Error]: ' + dbErr.message);
        }

        // Tentukan helper reply di awal agar tersedia untuk pengecekan rate limit
