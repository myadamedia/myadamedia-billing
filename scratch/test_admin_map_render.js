...
const htmlPath = path.join(__dirname, 'admin_map_verify.html');
const imgPath = path.join(__dirname, 'admin_map_verify.png');
fs.writeFileSync(htmlPath, rendered);

const chromePath = 'C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe';
execSync(`"${chromePath}" --headless=new --disable-gpu --window-size=1400,1200 --virtual-time-budget=4000 --screenshot="${imgPath}" "${htmlPath}"`, { stdio: 'inherit' });
