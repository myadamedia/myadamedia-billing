...
const outputScreenshot = path.join(baseDir, 'file md', 'screenshots', '06_peta_distribusi_odp.png');
...
const htmlPath = path.join(__dirname, 'temp_map_shot.html');
fs.writeFileSync(htmlPath, rendered);

const chromePath = 'C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe';
execSync(`"${chromePath}" --headless=new --disable-gpu --window-size=1280,800 --virtual-time-budget=4000 --screenshot="${outputScreenshot}" "${htmlPath}"`, { stdio: 'inherit' });

console.log('Updated 06_peta_distribusi_odp.png successfully, size:', fs.statSync(outputScreenshot).size);
