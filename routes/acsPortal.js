...
async function fetchDevicesFromACS(server, vParams = [], paths = {}, options = {}) {
    ...
    try {
        ...
        const customers = db.prepare('SELECT name, pppoe_username, genieacs_tag FROM customers').all();
        const pppoeMap = new Map();
        const tagMap = new Map();
        customers.forEach(c => {
            if (c.pppoe_username) {
                pppoeMap.set(c.pppoe_username.toLowerCase().trim(), c.name);
            }
            if (c.genieacs_tag) {
                tagMap.set(c.genieacs_tag.toLowerCase().trim(), c.name);
            }
        });
        ...
        const devices = devicesData.map(d => {
            ...
            // Customer Name lookup from DB mapping
            const lowerPppoe = pppoeUser ? pppoeUser.toLowerCase().trim() : '';
            const lowerId = d._id ? d._id.toLowerCase().trim() : '';
            const lowerSn = (d._deviceId?._SerialNumber || d._id) ? (d._deviceId?._SerialNumber || d._id).toLowerCase().trim() : '';
            const dbCustomerName = pppoeMap.get(lowerPppoe) || tagMap.get(lowerId) || tagMap.get(lowerSn);

            // Customer Name
            const customerName = dbCustomerName ||
                                getNestedValue(d, 'VirtualParameters.CustomerName') || 
...
router.get('/', async (req, res) => {
    try {
        ...
        if (targetServers.length > 0) {
            if (searchQuery) {
                // Find matching customers in SQLite first to get their PPPoE users / tags
                const dbCustomers = db.prepare('SELECT name, pppoe_username, genieacs_tag FROM customers WHERE name LIKE ?').all(`%${searchQuery}%`);
                const matchedPppoeUsers = dbCustomers.map(c => c.pppoe_username).filter(Boolean);
                const matchedTags = dbCustomers.map(c => c.genieacs_tag).filter(Boolean);
                ...
                matchedPppoeUsers.forEach(u => {
                    orConditions.push({ 'VirtualParameters.pppoeUsername': u });
                    orConditions.push({ 'VirtualParameters.pppUsername': u });
                    orConditions.push({ 'InternetGatewayDevice.WANDevice.1.WANConnectionDevice.1.WANPPPConnection.1.Username': u });
                    orConditions.push({ 'Device.PPP.Interface.1.Username': u });
                });

                matchedTags.forEach(t => {
                    orConditions.push({ '_id': t });
                    orConditions.push({ '_tags': t });
                });

                const query = JSON.stringify({ $or: orConditions });
                
                // Fetch customer list from local sqlite DB to map names
                const customers = db.prepare('SELECT name, pppoe_username, genieacs_tag FROM customers').all();
                const pppoeMap = new Map();
                const tagMap = new Map();
                customers.forEach(c => {
...
router.get('/device/:deviceId', async (req, res) => {
    try {
        ...
        const lowerPppoe = pppoeUser ? pppoeUser.toLowerCase().trim() : '';
        const lowerId = deviceData._id ? deviceData._id.toLowerCase().trim() : '';
        const lowerSn = (deviceData._deviceId?._SerialNumber || deviceData._id) ? (deviceData._deviceId?._SerialNumber || deviceData._id).toLowerCase().trim() : '';
        
        const dbCustomer = db.prepare('SELECT name FROM customers WHERE LOWER(pppoe_username) = ? OR LOWER(genieacs_tag) = ? OR LOWER(genieacs_tag) = ?').get(lowerPppoe, lowerId, lowerSn);
        const customerName = dbCustomer ? dbCustomer.name : (getNestedValue(deviceData, 'VirtualParameters.CustomerName') || getNestedValue(deviceData, 'VirtualParameters.customer_name') || '-');
...
// GET /api/analyze-down/:deviceId
router.get('/api/analyze-down/:deviceId', requireAdmin, async (req, res) => {
    try {
        const { deviceId } = req.params;
        const diagnosticsSvc = require('../services/diagnosticsService');
        const result = await diagnosticsSvc.analyzeAcsDeviceDown(deviceId);
        res.json(result);
    } catch (err) {
