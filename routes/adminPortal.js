...
router.get('/olts/:id/stats', requireAdminSession, async (req, res) => {
  try {
    ...
    if (stats) {
      ...
      const matchCustomer = (onu) => {
        ...
        if (snClean) {
          matched = customers.find(c => {
            const macClean = cleanStr(c.mac_address).replace(/[:-]/g, '');
            const snNoColons = snClean.replace(/[:-]/g, '');
      ...
      if (Array.isArray(stats.onus)) {
        stats.onus.forEach(onu => {
          const matched = matchCustomer(onu);
          onu.customer_name = matched ? matched.name : '-';
          onu.customer_id = matched ? matched.id : null;
        });
      }

      if (Array.isArray(stats.unauth_onus)) {
        stats.unauth_onus.forEach(onu => {
          const matched = matchCustomer(onu);
          onu.customer_name = matched ? matched.name : '-';
          onu.customer_id = matched ? matched.id : null;
        });
...
router.post('/billing/:id/whatsapp', requireAdminSession, async (req, res) => {
  try {
    ...
    if (isQrisCase && qrisJpgCaption && !qrisJpgCaption.includes(jatuhTempo)) {
      qrisJpgCaption = qrisJpgCaption.replace(/(Silakan scan|Terima kasih)/, `📅 *Jatuh Tempo:* ${jatuhTempo}\n\n$1`);
    }
    ...
    let formattedMsg = isQrisCase
      ...
      : template
        ...
        .replace(/{{nama}}/gi, customer.name || 'Pelanggan')
        .replace(/{{tagihan}}/gi, totalTagihan.toLocaleString('id-ID'))
        .replace(/{{sisa_tagihan}}/gi, (totalCarried > 0 ? totalCarried : totalTagihan).toLocaleString('id-ID'))
        .replace(/{{kurang_bayar}}/gi, totalTagihan.toLocaleString('id-ID'))
        .replace(/{{tagihan_berjalan}}/gi, (billingSummary.tagihanBerjalan || totalTagihan).toLocaleString('id-ID'))
        .replace(/{{sisa_lalu}}/gi, totalCarried.toLocaleString('id-ID'))
        .replace(/{{sisa_tagihan_bulan_lalu}}/gi, totalCarried.toLocaleString('id-ID'))
        .replace(/{{rincian_sisa}}/gi, rincianSisaText)
...
router.post('/tickets/:id/update', requireAdminSession, express.urlencoded({ extended: true }), async (req, res) => {
  try {
    ...
    let tech = null;
    if (newTechId) {
      tech = db.prepare('SELECT * FROM technicians WHERE id = ?').get(newTechId);
    }
    ...
    if (newTechId && newTechId !== prevTechId) {
      try {
        ...
        logger.error(`[AdminPortal] Gagal kirim WA penugasan teknisi: ${waErr.message}`);
      }
    }

    // --- WHATSAPP NOTIFICATION FOR IN_PROGRESS STATUS ---
    if (status === 'in_progress' && prevTicket && (newTechId !== prevTechId || prevTicket.status === 'open')) {
      try {
        const settings = getSettings();
        if (settings.whatsapp_enabled && prevTicket.customer_phone) {
          const { sendWA } = await import('../services/whatsappBot.mjs');
        ...
        logger.error(`[AdminPortal] Gagal kirim WA update keluhan pelanggan: ${waErr.message}`);
      }
    }
...
router.get('/reports', requireAdminSession, requireSidebarMenuAccess('reports'), (req, res) => {
  ...
  const filterMonthVal = req.query.month;
  const filterMonth = (filterMonthVal && filterMonthVal !== 'all') ? parseInt(filterMonthVal) : null;
  const now = new Date();
  ...
  const mStr = filterMonth ? String(filterMonth).padStart(2, '0') : null;

  // hitung data tahunan (referensi atau default)
  ...
  // VARIABEL TERFILTER (Sesuai parameter filterMonth)
  let totalRevenue;
  let cashInSelected;
  let expensesSelected;
  let netProfitSelected;
  let pendingAmount;
  ...
  if (filterMonth) {
    // 1. Total revenue lunas untuk bulan terpilih
    const revMonthRow = db.prepare(
      "SELECT SUM(amount) as t FROM invoices WHERE status='paid' AND strftime('%Y', paid_at) = ? AND strftime('%m', paid_at) = ?"
    ).get(yStr, mStr);
    totalRevenue = Number(revMonthRow?.t || 0);

    // 2. Cash In bulan terpilih
    const revDirectMonth = db.prepare(
      "SELECT SUM(amount) as t FROM invoices WHERE status='paid' AND strftime('%Y', paid_at) = ? AND strftime('%m', paid_at) = ? AND (paid_by_name IS NULL OR paid_by_name NOT LIKE 'Agent %')"
    ).get(yStr, mStr);
    const revenueMonthDirect = Number(revDirectMonth?.t || 0);

    const agentDepMonth = db.prepare(
      "SELECT SUM(amount_buy) as t FROM agent_transactions WHERE type='topup' AND strftime('%Y', created_at) = ? AND strftime('%m', created_at) = ?"
    ).get(yStr, mStr);
    const agentDepositMonth = Number(agentDepMonth?.t || 0);

    const customCashMonth = db.prepare(
      "SELECT SUM(amount) as t FROM cash_in WHERE strftime('%Y', date) = ? AND strftime('%m', date) = ?"
    ).get(yStr, mStr);
    const customCashMonthVal = Number(customCashMonth?.t || 0);

    cashInSelected = revenueMonthDirect + agentDepositMonth + customCashMonthVal;

    // 3. Expenses bulan terpilih
    const expRegMonth = db.prepare(
      "SELECT SUM(amount) as t FROM expenses WHERE strftime('%Y', date) = ? AND strftime('%m', date) = ?"
    ).get(yStr, mStr);
    const expensesRegularMonthVal = Number(expRegMonth?.t || 0);

    expensesSelected = expensesRegularMonthVal;
    netProfitSelected = cashInSelected - expensesSelected;

    // 4. Piutang belum bayar periode terpilih
    const pendingMonthRow = db.prepare("SELECT SUM(amount) as t FROM invoices WHERE status='unpaid' AND period_year = ? AND period_month = ?").get(filterYear, filterMonth);
    ...
    recentPayments = db.prepare(`
      ...
      JOIN customers c ON i.customer_id = c.id
      WHERE i.status='paid' AND strftime('%Y', i.paid_at) = ? AND strftime('%m', i.paid_at) = ?
      ORDER BY i.paid_at DESC LIMIT 10
    `).all(yStr, mStr);

    // 7. Top Unpaid bulan terpilih
    topUnpaid = db.prepare(`
      SELECT c.name, c.phone, COUNT(*) as unpaid_count, SUM(i.amount) as total_unpaid
      FROM invoices i JOIN customers c ON i.customer_id = c.id
      WHERE i.status='unpaid' AND i.period_year = ? AND i.period_month = ?
      GROUP BY c.id ORDER BY total_unpaid DESC LIMIT 5
    `).all(filterYear, filterMonth);
  } else {
    // Tampilan setahun penuh (perilaku lama)
    totalRevenue = revenueYearAll;
    cashInSelected = cashInYear;
    expensesSelected = expensesYear;
    netProfitSelected = netProfitYear;

    const pendingYearRow = db.prepare("SELECT SUM(amount) as t FROM invoices WHERE status='unpaid' AND period_year = ?").get(filterYear);
    pendingAmount = Number(pendingYearRow?.t || 0);
    ...
    recentPayments = db.prepare(`
      SELECT i.*, c.name as customer_name FROM invoices i
      JOIN customers c ON i.customer_id = c.id
      WHERE i.status='paid' AND strftime('%Y', i.paid_at) = ?
      ORDER BY i.paid_at DESC LIMIT 10
    `).all(yStr);
    ...
    topUnpaid = db.prepare(`
      SELECT c.name, c.phone, COUNT(*) as unpaid_count, SUM(i.amount) as total_unpaid
      FROM invoices i JOIN customers c ON i.customer_id = c.id
      WHERE i.status='unpaid' AND i.period_year = ?
      GROUP BY c.id ORDER BY total_unpaid DESC LIMIT 5
    `).all(filterYear);
  }
  ...
  res.render('admin/reports', {
    ...
    filterYear, filterMonth, monthlyData, chartData: monthlyData, recentPayments, topUnpaid,
    totalRevenue,
...
router.get('/reports/print', requireAdminSession, requireSidebarMenuAccess('reports'), (req, res) => {
  ...
  let cashInSelected = 0;
  let expensesSelected = 0;
  let netProfitSelected = 0;
  let expensesByCategory = [];
  ...
  if (filterMonth) {
    ...
    cashInSelected = revenueYearDirect + agentDepositYear + customCashInYear;
  ...
  res.render('admin/reports_print', {
    ...
    filterYear,
    filterMonth,
    cashInYear: cashInSelected,
    expensesYear: expensesSelected,
    netProfitYear: netProfitSelected,
    expensesByCategory,
    formatDateLocal
  });
});
...
router.get('/reports/export-csv', requireAdminSession, requireSidebarMenuAccess('reports'), (req, res) => {
  ...
  if (filterMonth) {
    ...
    expenses = db.prepare("SELECT date, category, amount, description FROM expenses WHERE strftime('%Y', date) = ? ORDER BY date ASC").all(yStr);
    cashIn = db.prepare("SELECT date, category, amount, description FROM cash_in WHERE strftime('%Y', date) = ? ORDER BY date ASC").all(yStr);
  }
  ...
  res.setHeader('Content-Disposition', `attachment; filename="Laporan_Keuangan_${periodStr.replace(/\s+/g, '_')}.csv"`);
  res.send(csvContent);
});
...
router.get('/inventory', requireAdminSession, requireSidebarMenuAccess('inventory'), (req, res) => {
  ...
  const customersList = db.prepare('SELECT id, name FROM customers ORDER BY name ASC').all();
...
router.get('/inventory/item/:id/stock', requireAdminSession, (req, res) => {
  try {
    const stock = inventorySvc.getStockByItem(req.params.id);
...
router.post('/inventory/stock/adjust', requireAdminSession, express.urlencoded({ extended: true }), (req, res) => {
  try {
    const { stock_id, quantity, note } = req.body;
    const actor = req.session.adminUser || 'Admin';
    inventorySvc.adjustStock(stock_id, Number(quantity), note, actor);
    req.session._msg = { type: 'success', text: 'Jumlah stok berhasil disesuaikan.' };
  } catch (e) {
    req.session._msg = { type: 'error', text: 'Gagal: ' + e.message };
  }
  res.redirect('/admin/inventory');
...
router.get('/api/mikrotik/system-resource', requireAdmin, async (req, res) => {
  try { res.json(await mikrotikService.getSystemResource(req.query.routerId)); } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.get('/whatsapp/web', requireAdminSession, requireSidebarMenuAccess('whatsapp'), async (req, res) => {
  ...
  try {
    ...
    if (req.body && req.body.auto_billing_days) {
      if (Array.isArray(req.body.auto_billing_days)) {
        autoBillingDays = req.body.auto_billing_days.join(',');
      } else {
        autoBillingDays = String(req.body.auto_billing_days);
      }
    } else {
      autoBillingDays = '';
    }
...
router.get('/api/whatsapp/chats', requireAdmin, (req, res) => {
  try {
    const chats = db.prepare(`
      SELECT jid, name, unread_count, last_message_text, last_message_timestamp
      FROM whatsapp_chats
      ORDER BY last_message_timestamp DESC
    `).all();

    // Ambil data pelanggan untuk pencocokan nama berdasarkan nomor telepon
    const customers = db.prepare(`
      SELECT name, phone FROM customers WHERE phone IS NOT NULL AND phone != ''
    `).all();

    // Helper untuk menormalisasi format nomor telepon
    const normalize = (phoneStr) => {
      if (!phoneStr) return '';
    ...
    // Petakan nomor telepon ternormalisasi ke nama pelanggan
    const customerMap = {};
    customers.forEach(c => {
      const norm = normalize(c.phone);
      if (norm) {
        customerMap[norm] = c.name;
      }
    });

    // Perkaya list chat dengan nama pelanggan asli dari database billing
    const enrichedChats = chats.map(chat => {
      const jidNumber = chat.jid.split('@')[0].split(':')[0];
      const customerName = customerMap[jidNumber];
      return {
        ...chat,
        name: customerName || chat.name || ''
      };
    });
...
router.get('/api/whatsapp/chats/:jid/messages', requireAdmin, (req, res) => {
  try {
    const { jid } = req.params;
    const messages = db.prepare(`
      SELECT id, chat_jid, from_me, text, timestamp, status
...
router.post('/api/whatsapp/chats/:jid/send', requireAdmin, express.json(), async (req, res) => {
  try {
    const { jid } = req.params;
    const { text } = req.body;
    if (!text) return res.status(400).json({ error: 'Teks pesan wajib diisi' });

    const { sendWA, whatsappStatus } = await import('../services/whatsappBot.mjs');
    if (whatsappStatus.connection !== 'open') {
      return res.status(400).json({ error: 'Bot WhatsApp tidak terhubung' });
    }

    // Kirim menggunakan socket Baileys
    await sendWA(jid, text);

    // Simpan pesan keluar ke database secara lokal
    const timestamp = Math.floor(Date.now() / 1000);
    const msgId = 'admin_' + Math.random().toString(36).substring(2) + '_' + timestamp;

    db.prepare(`
      INSERT INTO whatsapp_chats (jid, name, unread_count, last_message_text, last_message_timestamp)
      VALUES (?, '', 0, ?, ?)
      ON CONFLICT(jid) DO UPDATE SET
        last_message_text = excluded.last_message_text,
        last_message_timestamp = excluded.last_message_timestamp
    `).run(jid, text, timestamp);

    db.prepare(`
      INSERT OR IGNORE INTO whatsapp_messages (id, chat_jid, from_me, text, timestamp)
      VALUES (?, ?, 1, ?, ?)
    `).run(msgId, jid, text, timestamp);
...
router.post('/api/whatsapp/chats/:jid/read', requireAdmin, (req, res) => {
  try {
    const { jid } = req.params;
    db.prepare('UPDATE whatsapp_chats SET unread_count = 0 WHERE jid = ?').run(jid);
    res.json({ success: true });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
router.post('/api/whatsapp/chats/:jid/delete', requireAdmin, (req, res) => {
  try {
    const { jid } = req.params;
    db.prepare('DELETE FROM whatsapp_messages WHERE chat_jid = ?').run(jid);
    db.prepare('DELETE FROM whatsapp_chats WHERE jid = ?').run(jid);
    res.json({ success: true });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
